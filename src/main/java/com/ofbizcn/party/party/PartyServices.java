...
public class PartyServices {
	...
	public static Map<String, Object> deleteParty(DispatchContext ctx, Map<String, ? extends Object> context) {
		...
		return ServiceUtil.returnError(
				UtilProperties.getMessage(resourceError, "partyservices.cannot_delete_party_not_implemented", locale));
	}
	...
	public static Map<String, Object> createPerson(DispatchContext ctx, Map<String, ? extends Object> context) {
		...
		if (UtilValidate.isEmpty(partyId)) {
			try {
				...
				return ServiceUtil
						.returnError(UtilProperties.getMessage(resourceError, "party.id_generation_failure", locale));
			}
		}
		...
		if (party != null) {
			if (!"PERSON".equals(party.getString("partyTypeId"))) {
				return ServiceUtil.returnError(
						UtilProperties.getMessage(resourceError, "person.create.party_exists_not_person_type", locale));
			}
		} else {
			...
			Map<String, Object> newPartyMap = UtilMisc.toMap("partyId", partyId, "partyTypeId", "PERSON", "description",
					description, "createdDate", now, "lastModifiedDate", now, "statusId", statusId);
		...
		if (person != null) {
			return ServiceUtil
					.returnError(UtilProperties.getMessage(resourceError, "person.create.person_exists", locale));
		}
		...
		try {
			...
			Debug.logWarning(e.getMessage(), module);
			return ServiceUtil.returnError(UtilProperties.getMessage(resourceError, "person.create.db_error",
		...
		try {
			...
			if (!statusId.equals(oldStatusId)) {
				...
				if (oldStatusId == null) { // old records
					...
					GenericValue statusValidChange = EntityQuery.use(delegator).from("StatusValidChange")
							.where("statusId", party.getString("statusId"), "statusIdTo", statusId).queryOne();
					if (statusValidChange == null) {
						String errorMsg = "Cannot change party status from " + party.getString("statusId") + " to "
								+ statusId;
						Debug.logWarning(errorMsg, module);
						return ServiceUtil
								.returnError(
										UtilProperties.getMessage(resource,
												"PartyStatusCannotBeChanged", UtilMisc.toMap("partyFromStatusId",
														party.getString("statusId"), "partyToStatusId", statusId),
										locale));
					}
				...
				GenericValue partyStatus = delegator.makeValue("PartyStatus",
						UtilMisc.toMap("partyId", partyId, "statusId", statusId, "statusDate", statusDate));
				partyStatus.create();

				// disable all userlogins for this user when the new status is
				// disabled
				if (("PARTY_DISABLED").equals(statusId)) {
					List<GenericValue> userLogins = EntityQuery.use(delegator).from("UserLogin")
			...
			return ServiceUtil.returnError(UtilProperties.getMessage(resourceError, "person.update.write_failure",
					new Object[] { e.getMessage() }, locale));
		}
	}
	...
	public static Map<String, Object> updatePerson(DispatchContext ctx, Map<String, ? extends Object> context) {
		...
		if (UtilValidate.isEmpty(partyId)) {
			return ServiceUtil.returnError(
					UtilProperties.getMessage(ServiceUtil.resource, "serviceUtil.party_id_missing", locale));
		}
		...
		try {
			...
			return ServiceUtil.returnError(UtilProperties.getMessage(resourceError, "person.update.read_failure",
					new Object[] { e.getMessage() }, locale));
		}
		...
		if (UtilValidate.isNotEmpty(context.get("statusId")) && !context.get("statusId").equals(oldStatusId)) {
			try {
				dispatcher.runSync("setPartyStatus", UtilMisc.toMap("partyId", partyId, "statusId",
	...
	public static Map<String, Object> createPartyGroup(DispatchContext ctx, Map<String, ? extends Object> context) {
		...
		try {
			...
			GenericValue party = EntityQuery.use(delegator).from("Party").where("partyId", partyId).queryOne();
			GenericValue partyGroupPartyType = EntityQuery.use(delegator).from("PartyType")
					.where("partyTypeId", "PARTY_GROUP").cache().queryOne();
			...
			if (party != null) {
				...
				if (UtilValidate.isNotEmpty(context.get("partyTypeId"))) {
					GenericValue desiredPartyType = EntityQuery.use(delegator).from("PartyType")
							.where("partyTypeId", context.get("partyTypeId")).cache().queryOne();
					if (desiredPartyType != null && EntityTypeUtil.isType(desiredPartyType, partyGroupPartyType)) {
						...
						return ServiceUtil.returnError(UtilProperties.getMessage(resource, "PartyPartyTypeIdNotFound",
								UtilMisc.toMap("partyTypeId", context.get("partyTypeId")), locale));
					}
				}

				Map<String, Object> newPartyMap = UtilMisc.toMap("partyId", partyId, "partyTypeId", partyTypeId,
			...
			GenericValue partyGroup = EntityQuery.use(delegator).from("PartyGroup").where("partyId", partyId)
					.queryOne();
			if (partyGroup != null) {
				return ServiceUtil.returnError(UtilProperties.getMessage(resourceError,
			...
			Debug.logWarning(e, module);
			return ServiceUtil.returnError(
					UtilProperties.getMessage(resourceError, "partyservices.data_source_error_adding_party_group",
	...
	public static Map<String, Object> updatePartyGroup(DispatchContext ctx, Map<String, ? extends Object> context) {
		...
		try {
			...
			Debug.logWarning(e, module);
			return ServiceUtil.returnError(
					UtilProperties.getMessage(resourceError, "partyservices.could_not_update_party_information_read",
							UtilMisc.toMap("errMessage", e.getMessage()), locale));
		}
		...
		try {
			...
			Debug.logWarning(e.getMessage(), module);
			return ServiceUtil.returnError(
					UtilProperties.getMessage(resourceError, "partyservices.could_not_update_party_information_write",
							UtilMisc.toMap("errMessage", e.getMessage()), locale));
		}
	...
	public static Map<String, Object> createAffiliate(DispatchContext ctx, Map<String, ? extends Object> context) {
		...
		if (UtilValidate.isNotEmpty(partyId) && partyId.matches("\\d+")) {
			return ServiceUtil.returnError(
					UtilProperties.getMessage(resourceError, "partyservices.cannot_create_affiliate_digit", locale));
		}
		...
		try {
			...
			Debug.logWarning(e.getMessage(), module);
			return ServiceUtil.returnError(
					UtilProperties.getMessage(resourceError, "partyservices.could_not_add_affiliate_info_write",
							UtilMisc.toMap("errMessage", e.getMessage()), locale));
		}
	...
	public static Map<String, Object> createPartyNote(DispatchContext dctx, Map<String, ? extends Object> context) {
		...
		if (noteId == null) {
			...
			try {
				...
				Debug.logError(e, e.getMessage(), module);
				return ServiceUtil.returnError(UtilProperties.getMessage(resource, "PartyNoteCreationError",
		...
		try {
			...
			result.put(ModelService.ERROR_MESSAGE,
					UtilProperties.getMessage(resourceError, "partyservices.problem_associating_note_with_party",
	 ...
	 * @return Map with the result of the service, the output parameters.
	 */
	public static Map<String, Object> getPartyFromExactEmail(DispatchContext dctx,
		...
		try {
			...
			if (c != null) {
				for (GenericValue pacm : c) {
					GenericValue party = delegator.makeValue("Party",
							UtilMisc.toMap("partyId", pacm.get("partyId"), "partyTypeId", pacm.get("partyTypeId")));
			...
			return ServiceUtil.returnError(
					UtilProperties.getMessage(resourceError, "partyservices.cannot_get_party_entities_read",
							UtilMisc.toMap("errMessage", e.getMessage()), locale));
		}
		if (parties.size() > 0)
	 ...
	 * @return Map with the result of the service, the output parameters.
	 */
	public static Map<String, Object> getPartyFromUserLogin(DispatchContext dctx,
			Map<String, ? extends Object> context) {
		...
		if (userLoginId.length() == 0)
			return ServiceUtil
					.returnError(UtilProperties.getMessage(resource, "PartyCannotGetUserLoginFromParty", locale));
		...
		try {
			Collection<GenericValue> ulc = EntityQuery.use(delegator).from("PartyAndUserLogin")
					.where(EntityCondition.makeCondition(EntityFunction.UPPER_FIELD("userLoginId"), EntityOperator.LIKE,
							EntityFunction.UPPER("%" + userLoginId.toUpperCase() + "%")))
					.orderBy("userLoginId").queryList();
			...
			if (ulc != null) {
				for (GenericValue ul : ulc) {
					GenericValue party = delegator.makeValue("Party",
							UtilMisc.toMap("partyId", ul.get("partyId"), "partyTypeId", ul.get("partyTypeId")));
	...
	public static Map<String, Object> getPartyFromPerson(DispatchContext dctx, Map<String, ? extends Object> context) {
		...
		if (firstName.length() == 0 && lastName.length() == 0) {
			return ServiceUtil.returnError(
					UtilProperties.getMessage(resourceError, "partyservices.both_names_cannot_be_empty", locale));
		}
		...
		try {
			EntityConditionList<EntityExpr> ecl = EntityCondition.makeCondition(EntityOperator.AND,
					EntityCondition.makeCondition(EntityFunction.UPPER_FIELD("firstName"), EntityOperator.LIKE,
							EntityFunction.UPPER("%" + firstName.toUpperCase() + "%")),
					EntityCondition.makeCondition(EntityFunction.UPPER_FIELD("lastName"), EntityOperator.LIKE,
							EntityFunction.UPPER("%" + lastName.toUpperCase() + "%")));
			Collection<GenericValue> pc = EntityQuery.use(delegator).from("Person").where(ecl)
					.orderBy("lastName", "firstName", "partyId").queryList();
			...
			if (pc != null) {
				for (GenericValue person : pc) {
					GenericValue party = delegator.makeValue("Party",
							UtilMisc.toMap("partyId", person.get("partyId"), "partyTypeId", "PERSON"));
	 ...
	 * @return Map with the result of the service, the output parameters.
	 */
	public static Map<String, Object> getPartyFromPartyGroup(DispatchContext dctx,
			Map<String, ? extends Object> context) {
		Map<String, Object> result = FastMap.newInstance();
		Delegator delegator = dctx.getDelegator();
		Collection<Map<String, GenericValue>> parties = FastList.newInstance();
		...
		if (groupName.length() == 0) {
			return ServiceUtil
					.returnError(UtilProperties.getMessage(resource, "PartyCannotGetPartyFromPartyGroup", locale));
		}
		...
		try {
			Collection<GenericValue> pc = EntityQuery.use(delegator).from("PartyGroup")
					...
					.orderBy("groupName", "partyId").queryList();
			...
			if (pc != null) {
				for (GenericValue group : pc) {
					GenericValue party = delegator.makeValue("Party",
							UtilMisc.toMap("partyId", group.get("partyId"), "partyTypeId", "PARTY_GROUP"));
	...
	public static Map<String, Object> createRoleType(DispatchContext dctx, Map<String, ? extends Object> context) {
		...
		try {
			...
			return ServiceUtil.returnError(UtilProperties.getMessage(resource, "PartyCannotCreateRoleTypeEntity",
					UtilMisc.toMap("errMessage", e.getMessage()), locale));
		}
		if (roleType != null) {
	...
	public static Map<String, Object> createPartyDataSource(DispatchContext ctx,
			Map<String, ? extends Object> context) {
		Delegator delegator = ctx.getDelegator();
		Locale locale = (Locale) context.get("locale");
		...
		try {
			...
			GenericValue party = EntityQuery.use(delegator).from("Party").where("partyId", partyId).queryOne();
			GenericValue dataSource = EntityQuery.use(delegator).from("DataSource").where("dataSourceId", dataSourceId)
					.queryOne();
			if (party == null || dataSource == null) {
				List<String> errorList = UtilMisc
						.toList(UtilProperties.getMessage(resource, "PartyCannotCreatePartyDataSource", locale));
				if (party == null) {
					errorList
							.add(UtilProperties.getMessage(resource, "PartyNoPartyFoundWithPartyId", locale) + partyId);
				}
				if (dataSource == null) {
					errorList.add(UtilProperties.getMessage(resource, "PartyNoPartyWithDataSourceId",
			...
			GenericValue partyDataSource = delegator.makeValue("PartyDataSource",
					UtilMisc.toMap("partyId", partyId, "dataSourceId", dataSourceId, "fromDate", fromDate));
	...
	public static Map<String, Object> findParty(DispatchContext dctx, Map<String, ? extends Object> context) {
		...
		// get the role types
		try {
			List<GenericValue> roleTypes = EntityQuery.use(delegator).from("RoleType").orderBy("description")
					.queryList();
			result.put("roleTypes", roleTypes);
		} catch (GenericEntityException e) {
			String errMsg = "Error looking up RoleTypes: " + e.toString();
			Debug.logError(e, errMsg, module);
			return ServiceUtil.returnError(UtilProperties.getMessage(resource, "PartyLookupRoleTypeError",
		...
		try {
			...
			if (UtilValidate.isNotEmpty(roleTypeId)) {
				GenericValue currentRole = EntityQuery.use(delegator).from("RoleType").where("roleTypeId", roleTypeId)
						.cache().queryOne();
		...
		// get party types
		try {
			List<GenericValue> partyTypes = EntityQuery.use(delegator).from("PartyType").orderBy("description")
					.queryList();
			result.put("partyTypes", partyTypes);
		} catch (GenericEntityException e) {
			String errMsg = "Error looking up PartyTypes: " + e.toString();
			Debug.logError(e, errMsg, module);
			return ServiceUtil.returnError(UtilProperties.getMessage(resource, "PartyLookupPartyTypeError",
					UtilMisc.toMap("errMessage", e.toString()), locale));
		}
		...
		try {
			...
			if (UtilValidate.isNotEmpty(partyTypeId)) {
				GenericValue currentPartyType = EntityQuery.use(delegator).from("PartyType")
						.where("partyTypeId", partyTypeId).cache().queryOne();
		...
		try {
			...
			if (UtilValidate.isNotEmpty(stateProvinceGeoId)) {
				GenericValue currentStateGeo = EntityQuery.use(delegator).from("Geo").where("geoId", stateProvinceGeoId)
						.cache().queryOne();
			...
			Debug.logError(e, errMsg, module);
			return ServiceUtil.returnError(UtilProperties.getMessage(resource, "PartyLookupStateProvinceGeoError",
					UtilMisc.toMap("errMessage", e.toString()), locale));
		}
		...
		if ("Y".equals(lookupFlag)) {
			...
			if (!"Y".equals(showAll)) {
				...
				if (UtilValidate.isNotEmpty(inventoryItemId) || UtilValidate.isNotEmpty(serialNumber)
					...
					dynamicView.addViewLink("PT", "II", Boolean.FALSE,
							ModelKeyMap.makeKeyMapList("partyId", "ownerPartyId"));
				}
				...
				if ("P".equals(extInfo)) {
					...
					if (stateProvinceGeoId != null && !"ANY".equals(stateProvinceGeoId)) {
						...
						andExprs.add(EntityCondition.makeCondition("stateProvinceGeoId", EntityOperator.EQUALS,
								stateProvinceGeoId));
					}
			...
			if (mainCond != null || "Y".equals(showAll)) {
				try {
					...
					EntityListIterator pli = EntityQuery.use(delegator).select(UtilMisc.toSet(fieldsToSelect))
							.from(dynamicView).where(mainCond).orderBy(orderBy).cursorScrollInsensitive()
							.fetchSize(highIndex).distinct().queryIterator();
					...
					Debug.logError(e, errMsg, module);
					return ServiceUtil.returnError(UtilProperties.getMessage(resource, "PartyLookupPartyError",
							UtilMisc.toMap("errMessage", e.toString()), locale));
				}
			} else {
				partyListSize = 0;
	...
	public static Map<String, Object> linkParty(DispatchContext dctx, Map<String, ? extends Object> context) {
		...
		if ("PARTY_DISABLED".equals(partyTo.get("statusId"))) {
			return ServiceUtil
					.returnError(UtilProperties.getMessage(resource, "PartyCannotMergeDisabledParty", locale));
		}
		...
		try {
			delegator.storeByCondition("PartyContactMech",
					UtilMisc.<String, Object> toMap("partyId", partyIdTo, "thruDate", now),
		...
		try {
			delegator.storeByCondition("PartyContactMechPurpose",
					UtilMisc.<String, Object> toMap("partyId", partyIdTo, "thruDate", now),
					EntityCondition.makeCondition("partyId", EntityOperator.EQUALS, partyId));
		} catch (GenericEntityException e) {
			Debug.logError(e, module);
		...
		try {
			delegator.storeByCondition("ProductStoreRole",
					UtilMisc.<String, Object> toMap("partyId", partyIdTo, "thruDate", now),
					EntityCondition.makeCondition("partyId", EntityOperator.EQUALS, partyId));
		} catch (GenericEntityException e) {
			Debug.logError(e, module);
	...
	public static Map<String, Object> importAddressMatchMapCsv(DispatchContext dctx,
			Map<String, ? extends Object> context) {
		Delegator delegator = dctx.getDelegator();
		Locale locale = (Locale) context.get("locale");
		...
		for (int i = 0; i < records.length; i++) {
			if (records[i] != null) {
				...
				if (map.length != 2 && map.length != 3) {
					return ServiceUtil
							.returnError(UtilProperties.getMessage(resource, "PartyImportInvalidCsvFile", locale));
				} else {
				...
				return ServiceUtil
						.returnError(UtilProperties.getMessage(resource, "PartyImportNoRecordsFoundInFile", locale));
			}
		}
	...
	public static Map<String, Object> findPartyById(DispatchContext ctx, Map<String, Object> context) {
		...
		boolean searchPartyFirst = UtilValidate.isNotEmpty(searchPartyFirstContext)
				&& "N".equals(searchPartyFirstContext) ? false : true;
		boolean searchAllId = UtilValidate.isNotEmpty(searchAllIdContext) && "Y".equals(searchAllIdContext) ? true
				: false;

		GenericValue party = null;
		List<GenericValue> partiesFound = null;
		try {
			partiesFound = PartyWorker.findPartiesById(delegator, idToFind, partyIdentificationTypeId, searchPartyFirst,
					searchAllId);
		} catch (GenericEntityException e) {
			Debug.logError(e, module);
			return ServiceUtil.returnError(e.getMessage());
		...
		try {
			for (final CSVRecord rec : fmt.parse(csvReader)) {
				...
				if (lastPartyId == null || !currentPartyId.equals(lastPartyId)) {
					...
					} else if (EntityQuery.use(delegator).from("RoleType").where("roleTypeId", rec.get("roleTypeId"))
							.queryOne() == null) {
					...
					if (UtilValidate.isNotEmpty(rec.get("contactMechTypeId")) && EntityQuery.use(delegator)
							.from("ContactMechType").where("contactMechTypeId", rec.get("contactMechTypeId")).cache()
							.queryOne() == null) {
						newErrMsgs.add("Line number " + rec.getRecordNumber() + ": partyId: " + currentPartyId
					...
					if (UtilValidate.isNotEmpty(rec.get("contactMechTypeId"))
						...
						if (UtilValidate.isEmpty(rec.get("countryGeoId"))) {
							...
							List<GenericValue> countryCheck = EntityQuery.use(delegator).from("Geo")
									.where("geoTypeId", "COUNTRY", "abbreviation", rec.get("countryGeoId")).queryList();
							if (countryCheck.size() == 0) {
								newErrMsgs.add("Line number " + rec.getRecordNumber() + " partyId: " + currentPartyId
										+ " Invalid Country code: " + rec.get("countryGeoId"));
							}
						}
						...
						if (!addParty) { // update party
							...
							if (UtilValidate.isNotEmpty(rec.get("companyPartyId"))) {
								List<GenericValue> companyCheck = EntityQuery.use(delegator).from("PartyIdentification")
										.where("partyIdentificationTypeId", "PARTY_IMPORT", "idValue",
												rec.get("partyId"))
										.queryList();
								if (companyCheck.size() == 0) { // update party
																// group
									// company does not exist so create
									Map<String, Object> companyPartyGroup = UtilMisc.toMap("partyId", newCompanyPartyId,
				...
				if (newPartyId != null && addParty && UtilValidate.isNotEmpty(currentContactMechTypeId)) {
					...
					Map<String, Object> emailAddress = UtilMisc.toMap("contactMechTypeId", "EMAIL_ADDRESS", "userLogin",
							userLogin);
					...
					Map<String, Object> postalAddress = UtilMisc.toMap("userLogin", (Object) userLogin); // casting
																											// is
																											// here
																											// necessary
																											// for
					...
					Map<String, Object> telecomNumber = UtilMisc.toMap("userLogin", (Object) userLogin); // casting
																											// is
																											// here
																											// necessary
																											// for
					...
					if (currentContactMechPurposeTypeId != null && ("TELECOM_NUMBER".equals(currentContactMechTypeId)
						...
						Debug.logInfo("===================================last:" + lastContactMechPurposeTypeId
								+ " current: " + currentContactMechPurposeTypeId + " t :" + telecomNumberChanged
								+ " p: " + postalAddressChanged + " e: " + emailAddressChanged + " result: "
