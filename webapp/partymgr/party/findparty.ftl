...
<div class="button-bar"><a href="<@noerpUrl>${createUrl}</@noerpUrl>" class="buttontext create">${uiLabelMap.CommonCreateNew}</a></div>
<div class="screenlet">
  <div class="screenlet-title-bar">
  	<ul>
		...
		<#if partyList?has_content>
		  <#if hideFields == "Y">
		      <li class="collapsed"><a href="<@noerpUrl>findparty?hideFields=N&sortField=${sortField!}${paramList}</@noerpUrl>" title="${uiLabelMap.CommonShowLookupFields}">&nbsp;</a></li>
		  <#else>
		      <li class="expanded"><a href="<@noerpUrl>findparty?hideFields=Y&sortField=${sortField!}${paramList}</@noerpUrl>" title="${uiLabelMap.CommonHideFields}">&nbsp;</a></li>
		  </#if>
		</#if>
	</ul>
    <br class="clear"/>
    ...
    <div id="findPartyParameters" <#if hideFields != "N"> style="display:none" </#if> >
      ...
      <#-- NOTE: this form is setup to allow a search by partial partyId or userLoginId; to change it to go directly to
          the viewprofile page when these are entered add the follow attribute to the form element:
           ...
           onsubmit="javascript:lookupParty('<@noerpUrl>viewprofile</@noerpUrl>');"
       -->
      <form method="post" name="lookupparty" action="<@noerpUrl>findparty</@noerpUrl>" class="basic-form">
...
<div class="screenlet">
  <div class="screenlet-title-bar">
  	<ul>
  		<li class="h3">${uiLabelMap.CommonSearchResults}</li>
  	</ul>
  	<br class="clear"/>
  </div>
  <div class="screenlet-body">
<#if partyList??>
  <#if partyList?has_content>
    ...
    <#assign viewIndexLast = Static["org.noerp.base.util.UtilMisc"].getViewLastIndex(partyListSize, viewSize) />
    <#assign messageMap = Static["org.noerp.base.util.UtilMisc"].toMap("lowCount", lowIndex, "highCount", highIndex, "total", partyListSize)/>
    ...
    <#if extInfo?default("") == "T">
        ...
        <td>
            <a href="<@noerpUrl>findparty</@noerpUrl>?<#if sortField?has_content><#if sortField == "createdDate">sortField=-createdDate<#elseif sortField == "-createdDate">sortField=createdDate<#else>sortField=createdDate</#if><#else>sortField=createdDate</#if>${paramList!}&VIEW_SIZE=${viewSize!}&VIEW_INDEX=${viewIndex!}" 
        ...
        <td>
            <a href="<@noerpUrl>findparty</@noerpUrl>?<#if sortField?has_content><#if sortField == "lastModifiedDate">sortField=-lastModifiedDate<#elseif sortField == "-lastModifiedDate">sortField=lastModifiedDate<#else>sortField=lastModifiedDate</#if><#else>sortField=lastModifiedDate</#if>${paramList!}&VIEW_SIZE=${viewSize!}&VIEW_INDEX=${viewIndex!}" 
    ...
    <#list partyList as partyRow>
      ...
      <tr valign="middle"<#if alt_row> class="alternate-row"</#if>>
        <td><a href="<@noerpUrl>viewprofile?partyId=${partyRow.partyId}</@noerpUrl>">${partyRow.partyId}</a></td>
        <td>
      ...
      <#assign mainRole = dispatcher.runSync("getPartyMainRole", Static["org.ofbiz.base.util.UtilMisc"].toMap("partyId", partyRow.partyId, "userLogin", userLogin))/>
        ...
        <td class="button-col align-float">
          <a href="<@noerpUrl>viewprofile?partyId=${partyRow.partyId}</@noerpUrl>">${uiLabelMap.CommonDetails}</a>
