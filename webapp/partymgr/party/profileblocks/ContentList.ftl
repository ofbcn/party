      ...
      <#if partyContent?has_content>
        <table class="basic-table" cellspacing="0">
          <#list partyContent as pContent>
            ...
            <tr>
              <td class="button-col"><a href="<@noerpUrl>EditPartyContents?contentId=${pContent.contentId}&amp;partyId=${pContent.partyId}&amp;partyContentTypeId=${pContent.partyContentTypeId}&amp;fromDate=${pContent.fromDate}</@noerpUrl>">${content.contentId}</a></td>
              ...
              <td class="button-col">
                <#if (content.contentName?has_content)>
                    <a href="<@noerpUrl>img/${content.contentName}?imgId=${(content.dataResourceId)!}</@noerpUrl>">${uiLabelMap.CommonView}</a>
                </#if>
                <form name="removePartyContent_${pContent_index}" method="post" action="<@noerpUrl>removePartyContent/viewprofile</@noerpUrl>">
