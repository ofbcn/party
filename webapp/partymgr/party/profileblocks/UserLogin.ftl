    ...
    <div class="screenlet-title-bar">
      <ul>
        ...
        <#if security.hasEntityPermission("PARTYMGR", "_CREATE", session)>
          <li><a href="<@noerpUrl>ProfileCreateNewLogin?partyId=${party.partyId}</@noerpUrl>">${uiLabelMap.CommonCreateNew}</a></li>
        </#if>
      </ul>
      <br class="clear" />
    </div>
        ...
        <table class="basic-table" cellspacing="0">
          <#list userLogins as userUserLogin>
            <tr>
              ...
              <td class="button-col">
                <#if security.hasEntityPermission("PARTYMGR", "_CREATE", session)>
                  <a href="<@noerpUrl>ProfileEditUserLogin?partyId=${party.partyId}&amp;userLoginId=${userUserLogin.userLoginId}</@noerpUrl>">${uiLabelMap.CommonEdit}</a>
                </#if>
                <#if security.hasEntityPermission("SECURITY", "_VIEW", session)>
                  <a href="<@noerpUrl>ProfileEditUserLoginSecurityGroups?partyId=${party.partyId}&amp;userLoginId=${userUserLogin.userLoginId}</@noerpUrl>">${uiLabelMap.SecurityGroups}</a>
                </#if>
              </td>
            </tr>
          </#list>
