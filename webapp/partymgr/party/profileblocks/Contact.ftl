    ...
    <div class="screenlet-title-bar">
      <ul>
        ...
        <#if security.hasEntityPermission("PARTYMGR", "_CREATE", session) || userLogin.partyId == partyId>
          <li><a href="<@noerpUrl>editcontactmech?partyId=${partyId}</@noerpUrl>">${uiLabelMap.CommonCreateNew}</a></li>
        </#if>
      </ul>
      <br class="clear" />
    </div>
            ...
            <tr>
              ...
              <td>
                ...
                <#if "POSTAL_ADDRESS" = contactMech.contactMechTypeId>
                  <#if contactMechMap.postalAddress?has_content>
                    ...
                    <#if postalAddress.geoPointId?has_content>
                      ...
                      <a href="javascript:popUp('<@noerpUrl>GetPartyGeoLocation?geoPointId=${postalAddress.geoPointId}&partyId=${partyId}</@noerpUrl>', '${popUptitle!}', '450', '550')" class="buttontext">${uiLabelMap.CommonGeoLocation}</a>
                    </#if>
                  </#if>
                ...
                <#elseif "EMAIL_ADDRESS" = contactMech.contactMechTypeId>
                  <div>
                    ...
                    <form method="post" action="<@noerpUrl>NewDraftCommunicationEvent</@noerpUrl>" onsubmit="javascript:submitFormDisableSubmits(this)" name="createEmail${contactMech.infoString?replace("&#64;","")?replace(".","")}">
                ...
                <#-- create cust request -->
                <#if custRequestTypes??>
                  <form name="createCustRequestForm" action="<@noerpUrl>createCustRequest</@noerpUrl>" method="post" onsubmit="javascript:submitFormDisableSubmits(this)">
              ...
              <td class="button-col">
                <#if security.hasEntityPermission("PARTYMGR", "_UPDATE", session) || userLogin.partyId == partyId>
                  <a href="<@noerpUrl>editcontactmech?partyId=${partyId}&amp;contactMechId=${contactMech.contactMechId}</@noerpUrl>">${uiLabelMap.CommonUpdate}</a>
                </#if>
                <#if security.hasEntityPermission("PARTYMGR", "_DELETE", session) || userLogin.partyId == partyId>
                  <form name="partyDeleteContact" method="post" action="<@noerpUrl>deleteContactMech</@noerpUrl>" onsubmit="javascript:submitFormDisableSubmits(this)">
