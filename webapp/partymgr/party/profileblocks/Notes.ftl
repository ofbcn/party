    ...
    <div class="screenlet-title-bar">
      <ul>
        ...
        <#if security.hasEntityPermission("PARTYMGR", "_NOTE", session)>
          <li><a href="<@noerpUrl>AddPartyNote?partyId=${partyId}</@noerpUrl>">${uiLabelMap.CommonCreateNew}</a></li>
        </#if>
      </ul>
      <br class="clear" />
    </div>
            ...
            <tr>
              <td>
                ...
                <#if noteRef.noteParty?has_content>
                  <div><b>${uiLabelMap.CommonBy}: </b>${Static["com.ofbizcn.party.party.PartyHelper"].getPartyName(delegator, noteRef.noteParty, true)}</div>
                </#if>
