...
<#if !mechMap.contactMech??>
  ...
  <#if !preContactMechTypeId?has_content>
    ...
    <form method="post" action="<@noerpUrl>editcontactmech</@noerpUrl>" name="createcontactmechform">
...
<#if mechMap.contactMechTypeId?has_content>
  <#if !mechMap.contactMech?has_content>
    ...
    <table class="basic-table" cellspacing="0">
      <form method="post" action="<@noerpUrl>${mechMap.requestName}</@noerpUrl>" name="editcontactmechform" id="editcontactmechform">
        ...
        <tr>
          ...
          <td>
            <table class="basic-table" cellspacing="0">
              <#if mechMap.partyContactMechPurposes?has_content>
                <#list mechMap.partyContactMechPurposes as partyContactMechPurpose>
                  ...
                  <tr>
                    ...
                    <td class="button-col">
                      <form name="deletePartyContactMechPurpose_${partyContactMechPurpose.contactMechPurposeTypeId}" method="post" action="<@noerpUrl>deletePartyContactMechPurpose</@noerpUrl>" >
                         <input type="hidden" name="partyId" value="${partyId}" />
              ...
              <tr>
                <form method="post" action="<@noerpUrl>createPartyContactMechPurpose</@noerpUrl>" name="newpurposeform">
                  <input type="hidden" name="partyId" value="${partyId}" />
                  <input type="hidden" name="DONE_PAGE" value="${donePage}" />
                  <input type="hidden" name="useValues" value="true" />
                  <input type="hidden" name="contactMechId" value="${contactMechId!}" />
    ...
    <tr>   
      ...
      <td>     
        <select name="countryGeoId" id="editcontactmechform_countryGeoId">
          ...
          <#else>
           <#assign defaultCountryGeoId = Static["org.noerp.base.util.UtilProperties"].getPropertyValue("general.properties", "country.geo.id.default")>
          </#if>
          <option selected="selected" value="${defaultCountryGeoId}">
            <#assign countryGeo = delegator.findOne("Geo",Static["org.noerp.base.util.UtilMisc"].toMap("geoId",defaultCountryGeoId), false)>
            ${countryGeo.get("geoName",locale)}
          </option>
        </select>
      </td>
    </tr>
    <#assign isUsps = Static["com.ofbizcn.party.contact.ContactMechWorker"].isUspsAddress(mechMap.postalAddress)>
    <tr>
  ...
  <div class="button-bar">
    <a href="<@noerpUrl>backHome</@noerpUrl>" class="smallSubmit">${uiLabelMap.CommonGoBack}</a>
