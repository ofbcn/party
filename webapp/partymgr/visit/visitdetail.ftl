  ...
  <div class="screenlet-body">
      <table class="basic-table" cellspacing="0">
        ...
        <tr>
          ...
          <td><a href="<@noerpUrl>viewprofile?partyId=${visit.partyId!}</@noerpUrl>">${visit.partyId!}</a> / <a href="<@noerpUrl>viewprofile?partyId=${visit.partyId!}</@noerpUrl>">${visit.userLoginId!}</a></td>
        </tr>
        <tr>
  ...
  <div class="screenlet-body">
      <#if serverHits?has_content>
        <div class="align-float">
          <span class="label">
            <#if 0 < viewIndex>
              <a href="<@noerpUrl>visitdetail?visitId=${visitId}&amp;VIEW_SIZE=${viewSize}&amp;VIEW_INDEX=${viewIndex-1}</@noerpUrl>" class="smallSubmit">${uiLabelMap.CommonPrevious}</a> |
            </#if>
            <#if 0 < listSize>
              ${lowIndex+1} - ${highIndex} ${uiLabelMap.CommonOf} ${listSize}
            </#if>
            <#if highIndex < listSize>
              | <a href="<@noerpUrl>visitdetail?visitId=${visitId}&amp;VIEW_SIZE=${viewSize}&amp;VIEW_INDEX=${viewIndex+1}</@noerpUrl>" class="smallSubmit">${uiLabelMap.CommonNext}</a>
            </#if>
          </span>
        </div>
        <br class="clear"/>
      ...
      <#if security.hasPermission("SEND_CONTROL_APPLET", session)>
        <table class="basic-table" cellspacing="0">
            <tr>
              ...
              <td>
                <form name="pushPage" method="get" action="<@noerpUrl>pushPage</@noerpUrl>">
            ...
            <tr>
              ...
              <td>
                <form name="setFollower" method="get" action="<@noerpUrl>setAppletFollower</@noerpUrl>">
                  <input type="hidden" name="followerSid" value="${visit.sessionId}" />
                  <input type="hidden" name="visitId" value="${visit.visitId}" />
